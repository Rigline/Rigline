...
- 2026-09-23: `Health` gains `ready` (D85). A live read found that a `moved` reaction's silent
  success looked identical to nothing having happened — Leo waited, saw no change, and accepted VS
  Code's own "Restart Extensions" prompt before the companion had finished, landing on the offer 8b
  already names for exactly that race. `ready` is a status the background flow lands on instead of
  `ok` once it has actually patched a newly landed version, so a glance answers whether it is safe
  to restart without having had to watch the transition happen.
