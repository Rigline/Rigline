/**
 * `prototype build`: the convenience preset that turns a plugin's TypeScript into the one file its
 * manifest points at.
 *
 * The contract a plugin meets is an output, not a toolchain (decisions.md, P6): one browser-target
 * ES module plus prototype.json. This is the shortest way to produce that output and nothing more.
 * Everything the entry imports is bundled in, `@prototype/plugin-api` included, because a plugin
...
import { UserError } from "@prototype/core";
...
export async function buildPlugin(
  ...
  if (!existsSync(manifestPath)) throw new UserError(`${dir} has no prototype.json`);
